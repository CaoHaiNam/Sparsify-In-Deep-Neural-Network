...
def get(seed=0,pc_valid=0.10, tasknum = 11):
    data={}
    taskcla=[]
    size=[3,32,32]

    if not os.path.isdir('../dat/binary_split_cifar100/') or not os.path.isdir('../dat/binary_cifar10/'):
        if not os.path.isdir('../dat/binary_split_cifar100/'):
            os.makedirs('../dat/binary_split_cifar100')
        if not os.path.isdir('../dat/binary_cifar10/'):
...
#     ids=list(range(1,11))
    ...
    return data,taskcla,size,[0]+ids