...
from networks.vbd_net import VBD_Layer
...
#             total_acc+=hits.sum().data.cpu().numpy()[0]
    ...
    def criterion(self, outputs, targets):
        ...
        for i, m in enumerate(self.model.PM):
            if isinstance(m, VBD_Layer):
                reg += m.kl_divergence() * self.model.PM[i-1].weight.numel()
                num += self.model.PM[i-1].weight.numel()

        return self.ce(outputs, targets) + self.lamb * reg / num
    ...
    def prune(self, model):
        ...
        for i, m in enumerate(model.PM):
            ...
            if isinstance(m ,VBD_Layer):
                ...
                if isinstance(model.PM[i-1], nn.Linear) and not mid:
                    mid = True
                    mask_in = mask_in.view(-1,1,1).expand(mask_in.size(0),model.smid,model.smid).contiguous().view(-1)

                mask_out = m.get_mask()
                m.mu.data = m.mu.data[mask_out]
                m.log_sigma2.data = m.log_sigma2.data[mask_out]

                model.PM[i-1].weight.data = model.PM[i-1].weight.data[mask_out][:, mask_in]
                model.PM[i-1].bias.data = model.PM[i-1].bias.data[mask_out]

                m.out_features, m.in_features = model.PM[i-1].weight.shape[0], model.PM[i-1].weight.shape[1]
                mask_in = mask_out
